            ...
            if (is_file("$path/$element")) {
                ...
                echo '<div class="col d-flex flex-column">
                        <a href="./index.php?path=' . $path . '/' . $element . '"><i class="fas fa-folder fa-5x"></i></a>
                                    <div class="infoCard">
                                    <button type="button" class="fas fa-edit btn btn-primary renameBtn" data-bs-toggle="modal" data-bs-target="#modalEdit" id="'.$element.'"></button>
                                        <p class=" fileName">' . $element . '</p>
                                    </div>
                                </div>';
            }
...
function folderSideBar()
{
    ...
    foreach ($folders as $key => $element) {
        if (is_dir("$my_dir/$element")) {
            echo '<a href="index.php?path=' . $my_dir . '/' . $element . '" class="sub-item"><i class="fas fa-folder"></i>' . $element . '<svg class="trash-icon" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
