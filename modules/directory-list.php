...
function displayFileList($fileToDisplay)
{
    ...
    <td><span>" . explode(".", $fileToDisplay)[0] . "</span><span class='text-uppercase text-black-50 p-2' style='font-size: 0.8rem'>" .  explode(".", $fileToDisplay)[1] . "</span><span>" . dropdownMenuFile($fileToDisplay) . "</span></td>
...
function displayFolderList($folderToDisplay)
{
    ...
    echo ("<tr><th scope='row'><svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' class='bi bi-folder' viewBox='0 0 16 16'>
        ...
        <td>" . "<span style='display: inline-block'><form method='post' action='./modules/open-folder.php' ><input type='submit' class='btn btn-link' name='folder-name' value=" . $folderToDisplay . " /></form></span>" . dropdownMenuFolder($folderToDisplay) . "</td><td></td>
